...
func TestFindSleepiestMinute(t *testing.T) {
	...
	guard, minute := findSleepiestMinute(tt)
	if guard != 99 {
		t.Errorf("expected guard 99 got %d", guard)
	}
	if minute != 45 {
		t.Errorf("expected minute 45, got %d", minute)
	}
}
...
func TestExam(t *testing.T) {
	...
	logs := read(f)
	events := eventStream(logs)
	guard, minute := findSleepiestMinute(events)
